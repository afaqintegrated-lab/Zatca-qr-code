...
class ZATCASimplifiedTaxInvoice
{
    ...
    public function getCertificateInfo(string $certificate_string): array
    {
        ...
        $cleaned_certificate_string = chunk_split($cleaned_certificate_string, 64, "\n");
        $wrapped_certificate_string = "-----BEGIN CERTIFICATE-----\n{$cleaned_certificate_string}-----END CERTIFICATE-----";

        $hash = $this->getCertificateHash(str_replace("\n", '', $cleaned_certificate_string));
        ...
        if ($res === false) {
            throw new \Exception('Failed to get public key from certificate: ' . openssl_error_string());
        }

        $cert = openssl_pkey_get_details($res);

        if ($cert === false) {
            throw new \Exception('Failed to get certificate details: ' . openssl_error_string());
        }
